...
import { getJobCategoriesCollection, withId, getJobsCollection } from "@/db";
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  if (!existing) {
    return NextResponse.json({ ok: false, error: "नौकरी श्रेणी नहीं मिली" }, { status: 404 });
  }
  ...
  if (typeof body.canonicalUrl === "string" && body.canonicalUrl.trim()) {
    ...
    patch.canonicalUrl = `/job-category/${slug}`;
  }
  ...
  if (typeof body.slug === "string" && body.slug.trim()) {
    ...
    const dup = await categories.findOne({ slug: slugVal });
  ...
  if (!category) return NextResponse.json({ ok: false, error: "नौकरी श्रेणी नहीं मिली" }, { status: 404 });
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const referenced = await jobs.findOne({ categoryId: targetId });
  if (referenced) {
    return NextResponse.json(
      { ok: false, error: "इस श्रेणी से जुड़ी नौकरियां हैं, श्रेणी हटाई नहीं जा सकती" },
      { status: 400 }
    );
  }

  const others = await categories.find({ _id: { $ne: targetId } }).limit(1).toArray();
  if (others.length === 0) {
    return NextResponse.json(
      { ok: false, error: "कम से कम एक नौकरी श्रेणी ज़रूरी है" },
      { status: 400 }
    );
  }

  await categories.deleteOne({ _id: targetId });
