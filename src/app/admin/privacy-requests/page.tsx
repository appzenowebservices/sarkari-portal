...
const STATUS_COLORS: Record<string, string> = {
  ...
  ADDITIONAL_INFORMATION_REQUIRED: "bg-orange-100 text-orange-800",
...
export default function AdminPrivacyRequestsPage() {
  ...
  return (
    <div>
      <div className="mb-7">
        <p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-saffron-600">Privacy & Data</p>
        <h1 className="mt-1 font-display text-3xl font-bold tracking-tight text-navy-950 sm:text-4xl">Privacy Requests</h1>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
        ...
        <StatCard label="Verify" value={stats.verificationRequired} accent="amber" />
        <StatCard label="Review" value={stats.underReview} accent="purple" />
        <StatCard label="Processing" value={stats.processing} accent="sky" />
        <StatCard label="Completed" value={stats.completed} accent="green" />
        <StatCard label="Rejected" value={stats.rejected} accent="rose" />
      </div>

      {requests.length === 0 ? (
        <EmptyRow text="अभी कोई प्राइवेसी अनुरोध नहीं" />
      ) : (
        <AdminTable
          data={requests}
          emptyText="अभी कोई प्राइवेसी अनुरोध नहीं"
          header={
            <tr>
          ...
          renderRow={(req) => (
            <tr key={req.id} className="transition-colors hover:bg-paper">
              <td className="px-3 py-3">
                <p className="font-extrabold text-ink">{req.requestId}</p>
              </td>
              <td className="px-3 py-3">
                <p className="font-extrabold text-ink">{req.fullName}</p>
                <p className="text-xs text-ink-soft">{req.mobile || req.accountId}</p>
              </td>
              <td className="px-3 py-3">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <Field label="Account ID"><input className={inputCls} value={editing.accountId || "—"} readOnly /></Field>
              ...
              <div className="mt-2">
                <Field label="Description"><textarea className={`${inputCls} min-h-[80px] resize-y`} value={editing.description} readOnly /></Field>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <Field label="Verified"><input className={inputCls} value={editing.verifiedAt ? new Date(editing.verifiedAt).toLocaleString("en-IN") : "—"} readOnly /></Field>
                <Field label="Review Started"><input className={inputCls} value={editing.reviewStartedAt ? new Date(editing.reviewStartedAt).toLocaleString("en-IN") : "—"} readOnly /></Field>
